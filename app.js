...
function formatOrderDetails() {
  ...
  for (let index = 0; index < items.length; index++) {
    if (items[index].quantity !== 0) {
      orderMessage += ${items[index].name} - Quantity: ${items[index].quantity}\n;
    }
  }

  orderMessage += \nTotal Amount: ${finalDollars}.${finalCents} dollars;
...
cartButton.onclick = () => {
  ...
  const recipientNumber = '9344327004';
  
  const whatsappLink = https://wa.me/${recipientNumber}?text=${encodeURIComponent(orderDetails)};
